...
fn cannot_vote_two_times(mut session: Session) -> TestResult {
    let (enroll_addr, _) = deploy_contracts(&mut session)?;
    ...
    session.call_with_address::<_, ()>(enroll_addr, "subscribe", NO_ARGS, Some(1))??;

    session.sandbox().build_blocks(2);
    ...
    // we vote the first time
    session.set_actor(BOB.into());
    let result = session.call::<_, VotingError>("vote_for", NO_ARGS, NO_ENDOWMENT)?;

    assert!(result.is_ok());

    // we cannot vote a second time
    let error =
        session.call_and_expect_error::<_, VotingError>("vote_for", NO_ARGS, NO_ENDOWMENT)?;
